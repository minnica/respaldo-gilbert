...
if($tipo == 'ingenieria' || $tipo == 'Admin') {
    switch($opcion){
        case 1:
        $consulta = "INSERT INTO tabla (modulo, taller, revision, marca, cantidad, nombre, peso_unitario, fecha_ingenieria) VALUES('K', '$taller', '$revision', '$marca', '$cantidad', '$nombre', '$peso_unitario', NOW())";            
        ...
        break;
        case 4:    
        $consulta = "SELECT * FROM tabla WHERE modulo='K'";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
