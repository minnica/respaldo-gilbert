...
if($tipo == 'ingenieria' || $tipo == 'Admin') {
    switch($opcion){
        case 1:
        $consulta = "INSERT INTO tabla (modulo, taller, revision, marca, cantidad, nombre, peso_unitario, fecha_ingenieria) VALUES('G1', '$taller', '$revision', '$marca', '$cantidad', '$nombre', '$peso_unitario', NOW())";            
        ...
        break;
        case 4:    
        $consulta = "SELECT * FROM tabla WHERE modulo='G1'";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
