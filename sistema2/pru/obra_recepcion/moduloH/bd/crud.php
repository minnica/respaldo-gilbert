...
if($tipo == 'obra' || $tipo == 'Admin') {
    switch($opcion){   
        ...
        break;
        case 4:    
        $consulta = "SELECT id_tabla, taller, revision, marca, cantidad, nombre, peso_unitario, recibido, montaje, status2 FROM tabla WHERE modulo='H' AND enviado='SI' AND liberado='SI'";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
