...
if($tipo == 'calidad' || $tipo == 'Admin') {
    switch($opcion){ 
        ...
        break;
        case 4:    
        $consulta = "SELECT id_tabla, taller, revision, marca, cantidad, nombre, peso_unitario, armado, soldadura, limpieza, pintura, status, laboratorio FROM tabla WHERE modulo='G2' AND contratista!='' AND liberado='SI'";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
