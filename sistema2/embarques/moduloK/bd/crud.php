...
if($tipo == 'embarques' || $tipo == 'Admin') {
    switch($opcion){
        ...
        break;
        case 4:    
        $consulta = "SELECT * FROM tabla WHERE modulo='K' AND armado='SI' AND soldadura='SI' AND limpieza='SI' AND pintura='SI' AND liberado='SI' ORDER BY enviado asc, remision asc ";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
    ...
    switch($opcion){
        case 1:    
        $consulta = "SELECT * FROM tabla WHERE modulo='K' AND armado='SI' AND soldadura='SI' AND limpieza='SI' AND pintura='SI' AND liberado='SI'";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
