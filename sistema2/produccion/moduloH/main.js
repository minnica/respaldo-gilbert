$(document).ready(function() {
    ...
    tablaUsuarios = $('#tablaUsuarios').DataTable({
        createdRow: function(row, data) {
            if (data["contratista"] ===  null && data["cancelados"] != 'SI') {
    ...
    $(document).on("click", ".btnEditar", function() {
        ...
        contratista = fila.find('td:eq(7)').text();
    ...
    $(document).on("click", ".btnBorrar", function() {
        ...
        opcion = 3;
        var respuesta = confirm("¿Está seguro de borrar el campo CONTRATISTA del registro " + id_tabla + "?");
        if (respuesta) {
            $.ajax({
                url: "bd/crud.php",
                type: "POST",
